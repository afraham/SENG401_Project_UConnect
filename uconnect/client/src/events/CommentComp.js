...
function CommentComp({ commentHistory }) {
  ...
  const handleSendComment = async (e) => {
    ...
    if (inputValue !== "") {
      // Check if input is an empty string, if so, do not send.
      ...
      if (userEmail === null) {
        ...
        const newComment = {
          // Format for new comment
          userEmail: userEmail,
          message: inputValue,
        };
        try {
          const response = await fetch(
            `http://localhost:8000/api/eventById/${eventId}`,
            {
        ...
        {Array.isArray(comments) &&
          comments.map((comment, index) => (
            <div className="chat-elements" key={index}>
              <p className="chat-email">{comment.userEmail} </p>
              <p className="chat-text"> {comment.message}</p>
            </div>
          ))}
      </div>
