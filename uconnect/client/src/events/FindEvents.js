			...
			if (response.ok) {
				...
				const eventsWithExpansionAndStatus = data.map((event) => {
					...
					if (event.userEmail === userEmail) {
						...
						event.maxPeople === event.spotsTaken &&
						event.pending.includes(userEmail)
					) {
						...
						Array.isArray(event.approved) &&
						event.approved.includes(userEmail)
					) {
	...
	return (
		<div>
			...
			{events.length === 0 && searchQuery.trim() === "" && (
				<div className="no-events-message">No events present</div>
			)}
									...
									<div className="right-align">
										<button
											className={`request-button ${
												event.buttonDisplay === "Pending"
													? "pending"
													: event.buttonDisplay === "Joined"
													? "joined"
													: event.buttonDisplay === "Add to Waitlist"
													? "waitlist"
										...
										>
											...
											{event.buttonDisplay === "Add to Waitlist" &&
												"Add to Waitlist"}
											{event.buttonDisplay === "Request To Join" &&
												"Request To Join"}
