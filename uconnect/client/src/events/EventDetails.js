				...
				for (let i = 0; i < event.approved.length; i++) {
					...
					try {
						...
						if (response.ok) {
							...
							console.log(
								"Could not find matching profile, this might not be an error!"
							);
										...
										{user.profileAvaiable ? (
											<div className="event-details-profile">
												...
												<div className="event-details-user-info">
													<h1 className="event-details-approved-name">
														{user.name}
													</h1>
													<h10 className="event-details-approved-bio">
														{user.bio}
													</h10>
													{Array.isArray(user.interests) &&
														user.interests.length > 0 && (
															<div
																className="event-details-approved-interests-box"
																key={index}
															>
																{user.interests.map(
